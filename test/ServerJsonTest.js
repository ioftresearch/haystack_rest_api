...
/**
 * ServerJsonTest -- this test requires an instance of the TestDatabase.
 * @constructor
 */
function ServerJsonTest() {}
ServerJsonTest.prototype = Object.create(Test.prototype);
module.exports = ServerJsonTest;
...
function verifyAuth() {
  HClient.open(uri, user, pass, 'JSON', function(err, res) {
    if (err) throw err;
    client = res;
    runTests();
  });
...
function verifyAbout() {
  client.about(function(err, r) {
    ...
    Test.verifyEq(r.getStr("productName"), "Node Haystack Toolkit");
    Test.verifyEq(HTimeZone.make(r.getStr("tz")), HTimeZone.DEFAULT);
...
function verifyFormats() {
  client.formats(function(err, g) {
    ...
    Test.verify(g.col("read") !== null);
    Test.verify(g.col("write") !== null);
...
ServerJsonTest.test = function(callback) {
  cb = callback;
